...
# --- Subir PDF y generar QR preview con manejo de errores ---
...
def firmar():
    try:
        ...
        if not pdf_file or not p12_file or not p12_password:
            raise ValueError("Faltan archivos o contraseña")

        # Leer P12 y validar contraseña
        ...
        return render_template("seleccionar_firma.html", pdf_file=pdf_file.filename, username=session.get('username'))

    except Exception as e:
        return render_template("index.html", error=f"Error al procesar archivos: {str(e)}", username=session.get('username'))

# --- Generar PDF firmado final con manejo de errores ---
...
def generar_pdf_firmado():
    try:
        ...
        if not pdf_file or not p12_file or not p12_password:
            raise ValueError("No se encontraron archivos en sesión")
        ...
        return render_template("index.html", error=f"Error al generar PDF firmado: {str(e)}", username=session.get('username'))
